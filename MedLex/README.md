...
├── tunnel/                    # Redirections (anciennes URLs Next.js / export)
├── old/                       # Archives landings atelier (index-1…5, index-0)
├── au-clair/                  # Prototype Next.js optionnel (npm run dev)
...
| `tunnel/` | Fichiers de redirection uniquement ; régénérer avec `npm run sync:redirects` dans `au-clair/` |
| `au-clair/` | Prototype React ; la maquette officielle est `parcours/*.html` |
