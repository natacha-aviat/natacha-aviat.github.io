...
function getAudienceCategories(item) {
  if (Array.isArray(item.audienceCategories) && item.audienceCategories.length > 0) {
    return finalizeAudienceCategories(item.audienceCategories);
  }
  if (item.audienceCategory) {
    return finalizeAudienceCategories([item.audienceCategory]);
  }
...
function populateAudienceFilterButtons(options) {
  ...
  for (const group of groups) {
    ...
    groupEl.appendChild(buttonsEl);
    audienceFilterButtonsEl.appendChild(groupEl);
  }
}
...
const AGE_FILTER_IDS = new Set([...AGE_AUDIENCE_CATEGORIES]);
...
function passesAgeConstraints(categories, selected) {
  ...
  return [...ageSelected].some((filter) => categories.includes(filter));
}
...
function passesGeoConstraints(categories, selected) {
  ...
  return [...geoSelected].some((filter) => categories.includes(filter));
}
